...
public class MainActivity extends AppCompatActivity implements DatePickerDialog.OnDateSetListener {
    ...
    public void updateViews() {
        ...
        //textViewDaysSince.setText("Days since quarantine began on " + startDate + " at " + startTime);
        SpannableStringBuilder date = new SpannableStringBuilder(startDate);
        date.setSpan(new android.text.style.StyleSpan(android.graphics.Typeface.BOLD),0, date.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        SpannableStringBuilder time = new SpannableStringBuilder(startTime);
        time.setSpan(new android.text.style.StyleSpan(android.graphics.Typeface.BOLD),0, time.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);

        textViewDaysSince.setText("Days since quarantine began on ");
        textViewDaysSince.append(date);
        textViewDaysSince.append(" at ");
        textViewDaysSince.append(time);

        TextView textViewElapsedTime = (TextView) findViewById(R.id.text_view_elapsed_time);
        textViewElapsedTime.setText(days + " days, " + hours + " hours, " + mins + " mins");
    }
